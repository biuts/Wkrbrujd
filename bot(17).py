...
from database import (
    ...
    send_chat_message, get_chat_messages, delete_chat_message,
    get_new_chat_messages, edit_chat_message,
    update_complaint, delete_complaint,
    get_violator_complaint_count, get_admin_stats_weekly,
    get_audit_log,
...
MINI_APP_NAME = os.getenv("MINI_APP_NAME", "")
...
AUTH_DATE_TTL    = 86400  # секунд (24 часа)

# ── Rate limiter (in-memory, сброс каждый час) ────────────────
_rate_store: dict = defaultdict(lambda: {"count": 0, "reset": 0})
RATE_LIMIT_COMPLAINTS = 5   # жалоб в час на пользователя
RATE_LIMIT_WINDOW     = 3600

def _check_rate(user_id: int, bucket: str, limit: int) -> bool:
...
def mini_app_url(complaint_id=None):
    """Возвращает ссылку на мини-апп через t.me (с авторизацией Telegram)"""
    if BOT_USERNAME and MINI_APP_NAME:
        base = f"https://t.me/{BOT_USERNAME}/{MINI_APP_NAME}"
        if complaint_id:
            return f"{base}?startapp=complaint_{complaint_id}"
        return base
    # Fallback на прямой URL если не настроено
    if complaint_id:
        return f"{WEBAPP_URL}?complaint={complaint_id}"
    return WEBAPP_URL
...
def complaint_keyboard(complaint_id, status):
    # web_app кнопка — открывает с авторизацией Telegram (как /start)
    url = f"{WEBAPP_URL}?complaint={complaint_id}"
    buttons = [
...
async def notify_admins_new_complaint(complaint):
    ...
    reporter = html.escape(complaint.get("reporter_username") or complaint.get("reporter_name") or "Неизвестно")
    violator = html.escape(complaint.get("violator_username") or "—")
    desc = html.escape((complaint.get("description") or "")[:200])
    cid = complaint["id"]
...
async def cb_template(call: types.CallbackQuery):
    ...
    admin_name = html.escape(user.get("full_name") or f"@{user.get('username')}")
    try:
        await call.message.edit_text(
            call.message.text + f"\n\n{'─'*28}\n{status_str} · <i>{admin_name}</i>",
            parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔗 Открыть в приложении", url=mini_app_url(complaint_id))]])
        )
    except Exception as e:
        logger.warning(f"Не удалось обновить сообщение колбека: {e}")
...
async def cmd_start(message: types.Message):
    ...
    # WebAppInfo требует прямой HTTPS URL (не t.me ссылку)
...
def verify_telegram_data(init_data: str):
    try:
        ...
        if not hash_val:
            return None
        # Проверяем свежесть данных (не старше AUTH_DATE_TTL)
        auth_date = int(parsed.get("auth_date", 0))
        if time.time() - auth_date > AUTH_DATE_TTL:
...
async def api_create_complaint(request):
    ...
    # Rate limiting — не более RATE_LIMIT_COMPLAINTS жалоб в час
    if not _check_rate(user["telegram_id"], "complaint", RATE_LIMIT_COMPLAINTS):
        return json_response({"error": f"Слишком много жалоб. Попробуйте через час."}, 429)

    body = await request.json()
    ctype = (body.get("type") or "").strip()
    violator_username = (body.get("violator_username") or "").strip()[:MAX_USERNAME_LEN]
    description = (body.get("description") or "").strip()
    evidence = (body.get("evidence") or "").strip()[:MAX_EVIDENCE_LEN]

    # Валидация типа
    allowed_types = {"player", "admin", "suggest", "admin_apply"}
    if ctype not in allowed_types:
        return json_response({"error": "Неверный тип обращения"}, 400)
    if not violator_username or not description:
        return json_response({"error": "Заполните обязательные поля"}, 400)
    if len(description) > MAX_DESCRIPTION:
        return json_response({"error": f"Описание слишком длинное (макс. {MAX_DESCRIPTION} символов)"}, 400)

    cid = create_complaint(user["telegram_id"], ctype, violator_username,
                           body.get("violator_id"), description, evidence or None)
...
async def api_get_complaint(request):
    ...
    try:
        ...
        return json_response({"error": "Неверный ID"}, 400)
    complaint = get_complaint(cid)
...
async def api_chat_messages(request):
    ...
    before_id = request.rel_url.query.get("before_id")
    after_id = request.rel_url.query.get("after_id")
    try:
        if after_id:
            msgs = get_new_chat_messages(int(after_id))
        else:
            msgs = get_chat_messages(100, int(before_id) if before_id else None)
    except (ValueError, TypeError):
        return json_response({"error": "Неверный параметр"}, 400)
    for m in msgs:
...
async def api_chat_send(request):
    ...
    if not text: return json_response({"error": "Пустое сообщение"}, 400)
    if len(text) > 2000: return json_response({"error": "Слишком длинное сообщение"}, 400)
    reply_to = body.get("reply_to")
    result = send_chat_message(user["telegram_id"], text, reply_to)
    return json_response(result)

async def api_chat_edit(request):
    user = await auth_middleware(request)
    if not user: return json_response({"error": "Unauthorized"}, 401)
    try:
    ...
    if not text: return json_response({"error": "Пустое сообщение"}, 400)
    if len(text) > 2000: return json_response({"error": "Слишком длинное сообщение"}, 400)
    return json_response(edit_chat_message(user["telegram_id"], msg_id, text))
...
async def api_chat_delete(request):
    ...
    try:
        ...
        return json_response({"error": "Неверный ID"}, 400)
    return json_response(delete_chat_message(user["telegram_id"], msg_id))

# ── COMPLAINT EDIT / DELETE ────────────────────────────────

async def api_update_complaint(request):
    user = await auth_middleware(request)
    if not user: return json_response({"error": "Unauthorized"}, 401)
    if user["is_blocked"]: return json_response({"error": "Заблокирован"}, 403)
    try:
    ...
    description = (body.get("description") or "").strip()
    if not description:
        return json_response({"error": "Описание не может быть пустым"}, 400)
    if len(description) > MAX_DESCRIPTION:
        return json_response({"error": f"Описание слишком длинное (макс. {MAX_DESCRIPTION} символов)"}, 400)
    username = (body.get("violator_username") or "").strip()[:MAX_USERNAME_LEN] or None
    evidence = (body.get("evidence") or "").strip()[:MAX_EVIDENCE_LEN] or None
    return json_response(update_complaint(user["telegram_id"], complaint_id, description, username, evidence))

async def api_delete_complaint(request):
    user = await auth_middleware(request)
    if not user: return json_response({"error": "Unauthorized"}, 401)
    if user["is_blocked"]: return json_response({"error": "Заблокирован"}, 403)
    try:
        ...
        return json_response({"error": "Неверный ID"}, 400)
    return json_response(delete_complaint(user["telegram_id"], complaint_id))

# ── VIOLATOR REPEAT COUNT ──────────────────────────────────

async def api_violator_count(request):
    user = await auth_middleware(request)
    if not user: return json_response({"error": "Unauthorized"}, 401)
    if ROLES.get(user["role"], 0) < ADMIN_MIN_LEVEL:
        return json_response({"error": "Нет доступа"}, 403)
    username = request.rel_url.query.get("username", "")[:MAX_USERNAME_LEN]
    vid = request.rel_url.query.get("violator_id")
    try:
        vid_int = int(vid) if vid else None
    except ValueError:
        vid_int = None
    count = get_violator_complaint_count(username, vid_int)
...
async def api_admin_stats(request):
    ...
    if ROLES.get(user["role"], 0) < ADMIN_MIN_LEVEL:
        return json_response({"error": "Нет доступа"}, 403)
    stats = get_admin_stats_weekly()
    for s in stats:
        s["role_name"] = ROLE_NAMES.get(s["role"], s["role"])
...
async def cors_middleware(request, handler):
    ...
    if ALLOWED_ORIGIN and origin.startswith(ALLOWED_ORIGIN):
...
async def api_options(request):
    origin = request.headers.get("Origin", "")
    allow = origin if (ALLOWED_ORIGIN and origin.startswith(ALLOWED_ORIGIN)) else ("*" if not ALLOWED_ORIGIN else "")
...
def create_app():
    ...
    app.router.add_route("PATCH", "/api/complaint/{id}", api_update_complaint)
    app.router.add_delete("/api/complaint/{id}", api_delete_complaint)
    app.router.add_get("/api/violator-count", api_violator_count)
    app.router.add_get("/api/admin-stats", api_admin_stats)
    app.router.add_get("/api/audit-log", api_audit_log)
    app.router.add_get("/api/roles", api_roles)
    app.router.add_get("/api/chat", api_chat_messages)
    app.router.add_post("/api/chat", api_chat_send)
    app.router.add_delete("/api/chat/{id}", api_chat_delete)
    app.router.add_route("PATCH", "/api/chat/{id}", api_chat_edit)
    app.router.add_route("OPTIONS", "/{path_info:.*}", api_options)
    return app

async def keep_alive():
    """Пингует сервер каждые 14 минут чтобы не засыпал на Render Free"""
    import aiohttp
    await asyncio.sleep(60)  # подождать старт
    while True:
        try:
            async with aiohttp.ClientSession() as session:
                await session.get(f"{WEBAPP_URL}/api/roles", timeout=aiohttp.ClientTimeout(total=10))
            logger.info("Keep-alive ping OK")
        except Exception as e:
            logger.warning(f"Keep-alive ping failed: {e}")
        await asyncio.sleep(14 * 60)  # каждые 14 минут
