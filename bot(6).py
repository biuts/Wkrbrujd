...
def complaint_keyboard(complaint_id, status):
    ...
    buttons.append([InlineKeyboardButton(text="🔗 Открыть в приложении", url=f"{WEBAPP_URL}?complaint={complaint_id}")])
...
async def cb_template(call: types.CallbackQuery):
    ...
    # After setting "reviewing", show open-in-app button only
    new_kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔗 Открыть в приложении", url=f"{WEBAPP_URL}?complaint={complaint_id}")]])
    try:
