...
from database import (
    ...
    send_chat_message, get_chat_messages, delete_chat_message, get_new_chat_messages,
    edit_chat_message,
...
async def api_chat_edit(request):
    ...
    text = (body.get("text") or "").strip()
    if not text: return json_response({"error": "Пустое сообщение"}, 400)
    result = edit_chat_message(user["telegram_id"], msg_id, text)
    return json_response(result)
...
def create_app():
    ...
    app.router.add_patch("/api/chat/{id}", api_chat_edit)
    return app
