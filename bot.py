...
BOT_USERNAME = os.getenv("BOT_USERNAME", "")       # например: FrostbaneBot
MINI_APP_NAME = os.getenv("MINI_APP_NAME", "")     # например: reports (имя мини-апп в BotFather)
...
async def api_chat_delete(request):
    ...
    if not user: return json_response({"error": "Unauthorized"}, 401)
    msg_id = int(request.match_info["id"])
    result = delete_chat_message(user["telegram_id"], msg_id)
    return json_response(result)
...
async def api_update_complaint(request):
    ...
    return json_response(update_complaint(
        user["telegram_id"], complaint_id,
        description,
        body.get("violator_username"),
        body.get("evidence")
    ))
...
async def api_violator_count(request):
    ...
    count = get_violator_complaint_count(username, int(vid) if vid else None)
...
async def api_audit_log(request):
    user = await auth_middleware(request)
    if not user: return json_response({"error": "Unauthorized"}, 401)
    # Только senior_admin и выше (уровень 4+)
